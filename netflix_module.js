...
(function(){
  ...
  const CSS = `
    ...
    /* ── BOTÃO HOME FLUTUANTE (admin dentro de módulo) ── */
    #nf-admin-back{
      position:fixed;bottom:24px;left:24px;z-index:1000;
      display:none;align-items:center;gap:8px;
      background:#1c0e09;border:1px solid rgba(212,86,122,0.35);
      color:#FAF8F2;font-size:12px;font-weight:500;
      padding:10px 18px;border-radius:30px;cursor:pointer;
      box-shadow:0 4px 20px rgba(0,0,0,0.5);
      transition:all .2s;
    }
    #nf-admin-back:hover{background:#3a1020;border-color:#D4567A;transform:translateY(-2px);}
    ...
    .nf-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px;}
    ...
    .nf-card-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,0.85) 0%,rgba(0,0,0,0.2) 50%,transparent 100%);}
    ...
    .nf-card-badge{position:absolute;top:10px;right:10px;background:#C62828;color:#fff;font-size:9px;font-weight:600;padding:3px 7px;border-radius:20px;}
  ...
  // ── COVER STORAGE (localStorage — persiste por navegador) ────────────
  ...
  function openCoverModal(mod, onSave){
    ...
    modal.innerHTML = `
      <div class="nf-cover-box">
        ...
        <div class="nf-cover-preview" id="nf-cover-prev">
          ${current ? `<img src="${current}" onerror="this.parentElement.innerHTML='Imagem inválida'">` : 'Prévia aparece aqui'}
        </div>

        <div class="nf-cover-btns">
          <button class="nf-cover-save" id="nf-save-btn" onclick="NFModule._saveCover('${mod.id}')">Salvar</button>
          <button class="nf-cover-remove" onclick="NFModule._removeCover('${mod.id}')">Remover capa</button>
    ...
    // File input change
    document.getElementById('nf-file-input').addEventListener('change', e => {
      const f = e.target.files[0]; if(f) handleFile(f, mod.id);
    });
    ...
    document.getElementById('nf-cover-url').addEventListener('input', e => {
      ...
      prev.innerHTML = url ? `<img src="${url}" onerror="this.parentElement.innerHTML='Imagem inválida'">` : 'Prévia aparece aqui';
    });

    window._nfOnSave = onSave;
    window._nfCurrentModule = mod.id;
  }
  ...
  async function handleFile(file, moduleId){
    ...
    reader.onload = e => {
      prev.innerHTML = `<img src="${e.target.result}"><div class="nf-upload-progress" id="nf-upload-prog"><div class="nf-drop-icon">⏳</div><div>Fazendo upload...</div><div class="nf-progress-bar"><div class="nf-progress-fill" id="nf-prog-fill" style="width:10%"></div></div></div>`;
    };
  ...
  function _hideGrid(type){
    if(type === 'admin'){
      ...
      const existing = document.getElementById('nf-back-btn-client');
      if(!existing){
        const btn = document.createElement('div');
      ...
      } else { existing.style.display = 'inline-flex'; }
    }
  }
  ...
  function injectClient(){
    ...
    if(!document.getElementById('nf-client-home')){
      ...
      homeDiv.id = 'nf-client-home'; homeDiv.className = 'hidden';
      document.querySelector('.client-main')?.parentNode.insertBefore(homeDiv, document.querySelector('.client-main'));
    }
