...
(function () {
  ...
  window._AW2={
    ...
    async chatSend(){
      ...
      try{
        const key=window.ANTHROPIC_KEY||localStorage.getItem('primor_anthropic_key')||'';
        if(!key){
          document.getElementById('aw2td')?.remove();
          _addMsg('agent','⚠️ Chave Anthropic não configurada. Volte à aba Chat e configure a chave da API.');
